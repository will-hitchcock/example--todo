...
import {
  reducer, initialState, ADD_TODO, UPDATE_DRAFT, TOGGLE_STATUS,
} from '../ducks/todo/reducers';
...
const TodoList = () => {
  ...
  const toggleStatus = (idToUpdate, done) => () => dispatch(
    { type: TOGGLE_STATUS, idToUpdate, done },
  );
        ...
        <Ul>
          {map(({
            ...
            <Todo
              ...
              toggleStatus={toggleStatus(id, !done)}
            >
              {name}
            </Todo>
          ), todos.list)}
